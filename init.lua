...
minetest.register_privilege("testtools", {
	description = ("Allow player to use test tools"),
...
minetest.register_tool("testtools:node_setter", {
	...
	if priv then
		...
		if pointed_thing.type == "nothing" then
			...
			if player and player:is_player() then
				minetest.chat_send_player(player:get_player_name(), S("Now placing: @1 (param2=@2)", "air", 0))
			end
			return itemstack
		...
		if player and player:is_player() then
			minetest.chat_send_player(player:get_player_name(), S("Now placing: @1 (param2=@2)", node.name, node.param2))
		end
		return itemstack
	end
end,
	...
	if priv then
		...
		minetest.show_formspec(player:get_player_name(), "testtools:node_setter",
			"size[4,4]"..
	...
	if priv then
		...
		if nodename == "" and player and player:is_player() then
			minetest.chat_send_player(player:get_player_name(), S("Punch a node first!"))
			return
		end
		...
		if not minetest.registered_nodes[nodename] then
			minetest.chat_send_player(player:get_player_name(), S("Cannot set unknown node: @1", nodename))
			return
		end
	...
	if priv then
		...
		elseif pointed_thing.type == "object" then
			...
			if not obj:is_player() then
				...
				minetest.chat_send_player(player:get_player_name(), S("Can't remove players!"))
			end
		end
	end
end,
...
minetest.register_tool("testtools:falling_node_tool", {
	...
	groups = { testtool = 1, disable_repair = 1 },
	on_place = function(itemstack, player, pointed_thing)
	local t = "testtools:falling_node_tool"
	local name = player:get_player_name()
	local priv = minetest.check_player_privs(name, {testtools = true})
	local inv = player:get_inventory()
	if not priv and inv:contains_item("main", t) then
	...
	if priv then
		...
		if not ok and player and player:is_player() then
			minetest.chat_send_player(player:get_player_name(), S("Falling node could not be spawned!"))
		end
	end
end,
	on_use = function(itemstack, player, pointed_thing)
...
local mover_config = function(itemstack, player, pointed_thing)
	local t = "testtools:object_mover"
	local name = player:get_player_name()
	local priv = minetest.check_player_privs(name, {testtools = true})
	local inv = player:get_inventory()
	if not priv and inv:contains_item("main", t) then
	...
	minetest.chat_send_player(player:get_player_name(), S("distance=@1/10", dist*2))
	return itemstack
   end
end
...
minetest.register_tool("testtools:entity_scaler", {
	...
	groups = { testtool = 1, disable_repair = 1 },
	on_use = function(itemstack, player, pointed_thing)
	local t = "testtools:entity_scaler"
	local name = player:get_player_name()
	local priv = minetest.check_player_privs(name, {testtools = true})
	local inv = player:get_inventory()
	if not priv and inv:contains_item("main", t) then
...
minetest.register_tool("testtools:entity_spawner", {
	...
	groups = { testtool = 1, disable_repair = 1 },
	on_place = function(itemstack, player, pointed_thing)
	local t = "testtools:entity_spawner"
	local name = player:get_player_name()
	local priv = minetest.check_player_privs(name, {testtools = true})
	local inv = player:get_inventory()
	if not priv and inv:contains_item("main", t) then
...
minetest.register_tool("testtools:object_editor", {
	...
	groups = { testtool = 1, disable_repair = 1 },
	on_use = function(itemstack, player, pointed_thing)
	local t = "testtools:object_editor"
	local name = player:get_player_name()
	local priv = minetest.check_player_privs(name, {testtools = true})
	local inv = player:get_inventory()
	if not priv and inv:contains_item("main", t) then
...
local attacher_config = function(itemstack, player, pointed_thing)
	if not (player and player:is_player()) then
		local t = "testtools:object_attacher"
	local name = player:get_player_name()
	local priv = minetest.check_player_privs(name, {testtools = true})
	local inv = player:get_inventory()
	if not priv and inv:contains_item("main", t) then
